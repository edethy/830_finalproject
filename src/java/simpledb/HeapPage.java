...
public class HeapPage implements Page {
    ...
    */
    ...
    private int getHeaderSize() {                
    	int header_size = (int)(Math.ceil(getNumTuples()/8.0));
    	return header_size;                 
    }
    ...
    public boolean isSlotUsed(int i) {
    	int header_index = (int)Math.floor((i)/8);
    	if (header_index == getHeaderSize()) {
    		return true;
    	}
    	byte header_byte = this.header[header_index];
    	int offset = i % 8;
    	int filled = (header_byte >> offset) & 1;
    	return filled == 1;  
    }
    ...
    private void markSlotUsed(int i, boolean value) {
    	...
    	byte header_byte = this.header[header_index];
    	int offset = (i % 8);
    	if (value) { header_byte |= 1 << offset; }
    	else { header_byte &= ~(1 << offset); }
    	this.header[header_index] = header_byte;
    }
    ...
    public Iterator<Tuple> iterator() {
    	ArrayList<Tuple> full_tuple_array = new ArrayList<Tuple>();
        for (int i=0; i<this.tuples.length; i++) {
        	if (this.tuples[i] != null) {
        		full_tuple_array.add(this.tuples[i]);
        	}
        }
        return full_tuple_array.iterator();
    }
