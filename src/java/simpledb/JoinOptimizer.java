...
public class JoinOptimizer {
    ...
    PlanCache pc;
    HashMap<Integer, Set<BitSet>> bitset_map = new HashMap<Integer, Set<BitSet>>();
    ...
    public JoinOptimizer(LogicalPlan p, Vector<LogicalJoinNode> joins) {
        ...
        for (int i=0;i<15;i++) {
        	bitset_map.put(i, enumerateBitSet(i));
        }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        default:
        	double est_card =  (card1 * card2 * 0.3);
        	return (int)est_card;
        }
...
//        long start = System.currentTimeMillis();
    ...
    public Set<BitSet> enumerateBitSet(int total_size, int subset_size) {
    	
    	Set<BitSet> bitset_els = new HashSet<BitSet>();
    	
    	BitSet bs_one = new BitSet(1);
    	bs_one.set(0);
    	BitSet bs_zero = new BitSet(1);
    	bitset_els.add(bs_one);
    	bitset_els.add(bs_zero);
    	
    	for (int i=1;i<total_size;i++) {
    		Set<BitSet> update_bitset = new HashSet<BitSet>();
    		for (BitSet bs : bitset_els) {
    			BitSet bs_add_one = new BitSet(i+1);
    			bs_add_one.set(i);
    			BitSet bs_add_zero = new BitSet(i+1);
    			bs_add_zero.or(bs);
    			update_bitset.add(bs_add_zero);
    			bs_add_one.or(bs);
    			update_bitset.add(bs_add_one);
    			if (bs.cardinality() < subset_size) {
    				bs_add_one.or(bs);
    				update_bitset.add(bs_add_one);
    			}
    			
    		}
    		bitset_els = update_bitset;
    	}
    	Set<BitSet> to_return = new HashSet<BitSet>();
    	for (BitSet b : bitset_els) {
    		if(b.cardinality() == subset_size) {
    			to_return.add(b);
    		}
    	}
    	return to_return;
    ...
    public Set<BitSet> enumerateBitSet(int total_size) {
     ...
     * @return
     */
    public <T> Set<Set<T>> enumerateSubsetsOptimal(Vector<T> v, int size) {
    	
    	Set<Set<T>> subset_set = new HashSet<Set<T>>();
    	
    	Set<BitSet> bitset_subset = this.bitset_map.get(v.size());
    	if (bitset_subset == null) {
    		bitset_subset = enumerateBitSet(v.size());
    	}
    	
    	for (BitSet b : bitset_subset) {
    ...
    public Vector<LogicalJoinNode> orderJoins(
		...
		Set<LogicalJoinNode> node_set = new HashSet<LogicalJoinNode>(joins);
		System.out.println("Starting to order joins. Num Joins to order: " + node_set.size());
    	Vector<LogicalJoinNode> opt_plan = optJoin(node_set, stats, filterSelectivities);
    	if (explain) {
    ...
    @SuppressWarnings("unchecked")
    private Vector<LogicalJoinNode> optJoin(Set<LogicalJoinNode> subplan_set, HashMap<String, TableStats> stats, 
    										HashMap<String, Double> filterSelectivities) throws ParsingException {
    	Vector<LogicalJoinNode> subplan = new Vector<LogicalJoinNode>(subplan_set);
    	
    	if (pc.getOrder(subplan_set) != null) {
    		return pc.getOrder(subplan_set);
    	}
    	
    	if (subplan_set.size()==1) {
    		double best_plan_cost = Double.MAX_VALUE;
    		CostCard base_case = computeCostAndCardOfSubplan(stats, filterSelectivities, subplan.get(0), subplan_set, best_plan_cost, pc);
    		pc.addPlan(subplan_set, base_case.cost, base_case.card, base_case.plan);
    		return base_case.plan;
    	} else {
        	for (int i=1;i<=subplan.size();i++) {  
//        		Set<Set<LogicalJoinNode>> join_subset_set = enumerateSubsets(subplan, i);
        		Set<Set<LogicalJoinNode>> join_subset_set = enumerateSubsetsOptimal(subplan, i);
        		
        		for (Set<LogicalJoinNode> s : join_subset_set) {
                	
            		double best_plan_cost = Double.MAX_VALUE;
            		CostCard best_plan = new CostCard();
            		best_plan.cost = Double.MAX_VALUE;
            		
        			Vector<LogicalJoinNode> set_vector = new Vector<LogicalJoinNode>(s);
//         			Set<Set<LogicalJoinNode>> all_subsets = enumerateSubsets(set_vector, s.size()-1);
        			Set<Set<LogicalJoinNode>> all_subsets = enumerateSubsetsOptimal(set_vector, s.size()-1);
        			
        			
        			for (Set<LogicalJoinNode> s2 : all_subsets) {

        				
        		        Set<LogicalJoinNode> news = (Set<LogicalJoinNode>) ((HashSet<LogicalJoinNode>)s).clone();
        		        news.removeAll(s2);
        				Vector<LogicalJoinNode> remove_node = new Vector<LogicalJoinNode>(news);
        				
        				CostCard join_ss2 = computeCostAndCardOfSubplan(stats, filterSelectivities, remove_node.get(0), s, best_plan_cost, this.pc);
        				if (join_ss2 != null) {
        					best_plan_cost = join_ss2.cost;
        					best_plan = join_ss2;
        				}
        			}
        			if (best_plan.plan != null) {
        				pc.addPlan(s, best_plan.cost, best_plan.card, best_plan.plan);
        			} else {
        			}
